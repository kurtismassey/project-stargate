...
A remote viewing research platform. It runs protocol-faithful CRV, ERV, ARV, and WRV sessions with sealed double-blind targeting, records a typed multimodal transcript with research-scale instrumentation, scores sessions by independent rank-order judging (binary for ARV), and stores the sequential-series data that trans-temporal inhibition analyses need. Built the way a contractor lab would ship it if the SRI/CIA Star Gate program were funded today.
...
6. **Judgment.** A blind judge rank-orders the true target against pool decoys, or the two ARV associates. That rank is the score of record. The LLM analyst gives an advisory second opinion, never the official score.
...
- ERV, ARV, and WRV run as free-form variants with the same blindness, lock, and transcript rules. ARV seals an associate pair and judges both sides. WRV is written-first.
- Sequential series record trial positions so hits can be scored at lags -2 to +2 for TTI analysis. The ops console can seal a run of N trials in one cut.

...
A remote viewing research console for a single-operator lab. The operator seals targets into pools, cuts taskings with opaque cue numbers, and runs protocol-enforced sessions in a viewing chamber. The chamber keeps the viewer blind, walks CRV structure stage by stage, records a typed multimodal transcript with full timestamps, and locks before any feedback. After lock the target unseals, the analyst model gives an advisory read, and blind rank-order judging produces the official score. Series of sessions accumulate the displacement and latency data Tart's trans-temporal inhibition analysis needs.
...
- ERV sessions skip stage gating but keep blindness, lock, transcript, and timestamps [CIA-BRIEF].
...
- For series members, the same machinery scores lags -2 to +2 into `displacement_scores` [TART-TTI].
...
Auth and multi-operator tenancy, WRV, ARV execution (schema reserved), human-monitor pairing UX beyond live ink watching, outbound-beacon targets.

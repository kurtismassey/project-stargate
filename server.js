...
app.prepare().then(() => {
  ...
  gemini.on("connection", (socket) => {
    ...
    socket.on("queryGemini", async (messages) => {
      ...
      try {
        let prompt = messages.map(msg => `Viewer ${viewerId}: ${msg.text}`).join('\n');
        prompt = `Welcome to Project Stargate, are you ready to begin?\n` + prompt;
        ...
        socket.emit("geminiError", { message: "Error processing your request" });
      }
    });
